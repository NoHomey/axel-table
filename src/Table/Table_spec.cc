...
private:
    ...
    void forEachCellIndexInTable(Functor functor) const noexcept {
        table.getRows().forEach([&functor](const Row* rowPtr, size_t row) {
            rowPtr->getCells().forEach([row, &functor](const TableCell*, size_t col) {
                functor(row, col);
            });
        });
    }
...
TEST(Table, getRowsCountWhenTrailingRowsAreEmpty) {
    Table table;
    EXPECT_EQ(table.getRowsCount(), 0);

    table.edit({1, 2}, DivByZeroCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 2);

    table.edit({8, 7}, CircRefCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 9);

    table.edit({18, 6}, ErrorCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 19);

    table.edit({18, 6}, EmptyCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 9);

    table.edit({8, 7}, EmptyCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 2);

    table.edit({1, 2}, EmptyCell::obtainPtr());
    EXPECT_EQ(table.getRowsCount(), 0);
}